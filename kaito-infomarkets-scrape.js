...
const LEADERBOARD_DURATIONS = ["7d", "30d", "3m", "6m", "12m"];
...
async function collectLines(tickers) {
  ...
  for (const duration of LINE_DURATIONS) {
    ...
    const name = `infomarkets-lines-${duration}.json`;
...
async function main() {
  ...
  const lineFiles = await collectLines(mindshare.tickers);
  const leaderboard = await collectLeaderboard();
  ...
  manifest.files["infomarkets-mindshare.json"] = await writeJson(
    "infomarkets-mindshare.json",
    mindshare.results
  );
  manifest.files["infomarkets-tickers.json"] = await writeJson(
    "infomarkets-tickers.json",
    mindshare.tickers
  );
  manifest.files["infomarkets-leaderboard.json"] = await writeJson(
    "infomarkets-leaderboard.json",
    leaderboard
  );

  Object.assign(manifest.files, lineFiles);

  manifest.files["infomarkets-manifest.json"] = await writeJson(
    "infomarkets-manifest.json",
    manifest
  );
