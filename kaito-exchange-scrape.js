...
async function collectLines(tickers) {
  ...
  for (const duration of LINE_DURATIONS) {
    ...
    const name = `exchange-lines-${duration}.json`;
...
async function main() {
  ...
  manifest.files["exchange-mindshare.json"] = await writeJson(
    "exchange-mindshare.json",
    mindshare.results
  );
  manifest.files["exchange-tickers.json"] = await writeJson(
    "exchange-tickers.json",
    mindshare.tickers
  );

  Object.assign(manifest.files, lineFiles);

  manifest.files["exchange-manifest.json"] = await writeJson(
