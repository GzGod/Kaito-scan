...
const LEADERBOARD_DURATIONS = ["24h", "7d", "30d"];
...
async function collectMindshare() {
  const results = {};
...
async function collectLines(tickers) {
  const results = {};
  ...
  for (const duration of LINE_DURATIONS) {
    for (const ticker of tickers) {
      ...
      await writeJson(`pre-tge-lines-${duration}.json`, results);
      await sleep(REQUEST_DELAY_MS);
    }
  }
...
async function collectLeaderboard() {
  ...
  for (const duration of LEADERBOARD_DURATIONS) {
    ...
    console.log(`[leaderboard] ${key}`);
  }

  return results;
}

async function collectTopPreTge() {
  return fetchProtectedJson(BASE_URL, "kol/mindshare/top-pre-tge", {}, "top-pre-tge");
}

async function main() {
  const startedAt = new Date().toISOString();

  const mindshare = await collectMindshare();
  const lines = await collectLines(mindshare.tickers);
  ...
  let topPreTge;
  try {
    topPreTge = await collectTopPreTge();
  } catch (error) {
    topPreTge = { error: error.message };
  }
  ...
  manifest.files["pre-tge-mindshare.json"] = await writeJson("pre-tge-mindshare.json", mindshare.results);
  manifest.files["pre-tge-tickers.json"] = await writeJson("pre-tge-tickers.json", mindshare.tickers);
  manifest.files["pre-tge-lines.json"] = await writeJson("pre-tge-lines.json", lines);
  manifest.files["pre-tge-leaderboard.json"] = await writeJson("pre-tge-leaderboard.json", leaderboard);
  manifest.files["pre-tge-top-pre-tge.json"] = await writeJson("pre-tge-top-pre-tge.json", topPreTge);
